...
const game = {
    ...
    customers: [],
    stations: [
        { id: 1, name: 'Bean Extraction', icon: '🥤', slots: [null], maxSlots: 1 },
        { id: 2, name: 'Desserts', icon: '🥠', slots: [null], maxSlots: 1 },
        { id: 3, name: 'ATM', icon: '🏦', slots: [null], maxSlots: 1 },
        { id: 4, name: 'Study Services', icon: '🎒', slots: [null], maxSlots: 1 }
    ],
    customerIdCounter: 0,
    upgrades: {
        limitedCups: { level: 0, maxLevel: 3 },
        intDesserts: { level: 0, maxLevel: 3 },
        wifiStation: { level: 0, maxLevel: 3 },
        loyalty: { level: 0, maxLevel: 3 }
    },
    passiveIncome: 0,
...
// Service definitions
const services = [
    { name: 'Bean Extraction', icon: '🥤', duration: 5000, price: 5, station: 1 },
    { name: 'Desserts', icon: '🥠', duration: 6000, price: 6, station: 2 },
    { name: 'ATM', icon: '🏦', duration: 7000, price: 10, station: 3 },
    { name: 'Study Services', icon: '🎒', duration: 8000, price: 15, station: 4 }
];

// Upgrade definitions
const upgradeDefinitions = {
    limitedCups: {
        name: 'Limited Edition Cups',
        icon: '🍶',
        description: 'Comfortable massage chairs for waiting customers',
        baseCost: 150,
        costMultiplier: 1.5,
        incomePerLevel: 5,
        patienceBonus: 10,
    ...
    intDesserts: {
        name: 'International Desserts',
        icon: '🍡',
        description: 'Delicious desserts from overseas keep customers happy and generate income',
        baseCost: 300,
        costMultiplier: 1.6,
        incomePerLevel: 8,
        patienceBonus: 15,
    ...
    wifiStation: {
        name: 'Wifi Station',
        icon: '🛜',
        description: 'Free WiFi for your customers',
        baseCost: 250,
        costMultiplier: 1.5,
        incomePerLevel: 6,
        patienceBonus: 12,
    ...
    loyalty: {
        ...
        icon: '🪪',
        description: 'Customers can enjoy benefits, increasing tips!',
        baseCost: 100,
        costMultiplier: 2,
        incomePerLevel: 15,
        tipBonus: 5,
...
// Customer names
const customerNames = [
    'Sophia', 'Emma', 'Olivia', 'Ava', 'Isabella', 'Miza', 'Charlotte', 
    'Amelia', 'Harper', 'Evelyn', 'Darleze', 'Emily', 'Luna', 'Aria',
    'Scarlett', 'Madison', 'Chloe', 'Grace', 'Victoria', 'Lily'
];
...
function calculatePatienceBonus() {
    let bonus = 0;
    if (game.upgrades.limitedCups.level > 0) {
        bonus += upgradeDefinitions.limitedCups.patienceBonus * game.upgrades.limitedCups.level;
    }
    if (game.upgrades.intDesserts.level > 0) {
        bonus += upgradeDefinitions.intDesserts.patienceBonus * game.upgrades.intDesserts.level;
    }
    if (game.upgrades.wifiStation.level > 0) {
        bonus += upgradeDefinitions.wifiStation.patienceBonus * game.upgrades.wifiStation.level;
    }
    return bonus;
}
...
function calculateTipBonus() {
    let bonus = 0;
    if (game.upgrades.loyalty.level > 0) {
        bonus += upgradeDefinitions.loyalty.tipBonus * game.upgrades.loyalty.level;
    }
    return bonus;
}
